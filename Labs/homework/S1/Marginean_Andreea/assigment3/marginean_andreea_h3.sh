...
# The script will display the name of the file which contains the highest number of words and the word count.
#!/bin/bash
...
for file in "$@"; do
    if [ -f "$file" ]; then
        ...
        echo "Warning: '$file' is not a valid file or does not exist."
    fi
done
